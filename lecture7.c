...
typedef struct queue_t
{
	...
	int items[QUEUE_MAX];										// reason why we don't have this grow - we're lazy; more complicated to make it thread-safe while growing / shrinking array
																// fine to use fixed-size arrays at end of game; dynamic size can lead to unpredictable memory requirements
	int head_index;
	int tail_index;
} queue_t;
...
static void queue_push(queue_t* queue, int item)
{
	...
	// push to rear
	// releasing here makes pop start reading from queue; nothing might be there
	int index = atomic_increment(&queue->tail_index) % QUEUE_MAX;			// this allows us to go above 16 in our count, but keep our indices 0 to 15; atomic b/c multiple threads could increment at same time
	queue->items[index] = item;
	semaphore_release(queue->used_items);										// increment b/c count acts as a count of everything currently in queue

	// only other concern is if tail overtakes head
}
...
static int queue_pop(queue_t* queue)
{
	...
	int item = queue->items[index];											// need this here because otherwise can become a data race in the return line, wehre push can overwrite what we want to return
...
void lecture7_queue_test()
{
	queue_t queue =
	{
		...
		.free_items = semaphore_create(0, QUEUE_MAX),
	};
